...
### ✅ External Secrets Operator - Integração com OCI Vault (Jan 2026)

Implementada integração completa com OCI Vault para gerenciamento seguro de secrets:

**Melhorias implementadas:**
- ✅ External Secrets Operator configurado com Instance Principal
- ✅ Template Helm para SecretStore e ExternalSecret
- ✅ Controle via variável GitHub `EXTERNAL_SECRETS_ENABLED`
- ✅ Uso de **nomes** de secrets ao invés de OCIDs (simplificação)
- ✅ Endpoint `/db-status` para validação de secrets injetados
- ✅ Documentação e exemplos em `examples/external-secrets/`

**IAM Policies necessárias (tenancy level):**
```hcl
Allow dynamic-group <your-dynamic-group> to read vaults in tenancy
Allow dynamic-group <your-dynamic-group> to read keys in tenancy
Allow dynamic-group <your-dynamic-group> to read secret-bundles in tenancy
Allow dynamic-group <your-dynamic-group> to use secret-family in tenancy
```

**Configuração no Helm:**
```yaml
externalSecrets:
  enabled: true  # Controlado via GitHub Variable
  vaultId: "ocid1.vault.oc1..."  # Passado via --set no workflow
  region: "sa-saopaulo-1"
  secrets:
    - key: DATABASE_PASSWORD
      name: database-password  # Nome do secret no Vault (não OCID)
```

### ✅ Image Pull Policy - Sempre atualizado (Jan 2026)

Corrigido `imagePullPolicy` para garantir que imagens sejam sempre atualizadas:

**Antes:**
```yaml
pullPolicy: IfNotPresent  # ❌ Usava cache do node
```

**Depois:**
```yaml
pullPolicy: Always  # ✅ Sempre faz pull da nova imagem
```

**Impacto:** Garante que deploys sempre usem a versão mais recente da imagem, mesmo com tags `latest`.

### ✅ Smoke Tests - Imagens Fully Qualified (Jan 2026)

Corrigido erro de "ambiguous short name" nos smoke tests:

**Antes:**
```bash
kubectl run smoke-test --image=curlimages/curl:latest
# ❌ Erro: short name mode is enforcing
```

**Depois:**
```bash
kubectl run smoke-test --image=docker.io/curlimages/curl:latest
# ✅ Funciona corretamente
```

**Arquivos atualizados:**
- `.github/workflows/cd-dev.yml`
- `.github/workflows/cd-staging.yml` (se existir)
- `.github/workflows/cd-production.yml` (se existir)
